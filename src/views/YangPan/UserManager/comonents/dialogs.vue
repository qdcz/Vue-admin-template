    ...
    <el-dialog title="修改个人信息" :visible.sync="isshowDialogs" width="60%" center @close="dialogClose">
      <el-form ref="form" :model="form" class="el-form" :rules="rules" label-width="80px" label-position="left">
        <el-form-item label="账号" prop="account">
          <el-input v-model="form.account" size="large" disabled="disabled" />
        </el-form-item>
        <el-form-item label="密码" prop="pwd" v-if="form.pwd">
...
<script>
  ...
  import {
    API$GetSts,
    API$DelOssFile
  } from '../../../../api/YangPan/OSS.js'
  import {
    API$UpdUser
  } from '../../../../api/YangPan/User.js'
      ...
      async AddAPI() {
        try {
          const {
            code,
            msg
          } = await API$AddUpdateVersion(this.form)
          if (code === 200) {
            this.$message(msg)
            this.dialogClose()
      ...
      async singleFileChange(file, fileList) {
        ...
        try {
          const res = await this.ali_oss.multipartUpload(
            `/uploadImg/${this.form.account}/${name}_${new Date().getTime()}.${type}`, copyFile, {
